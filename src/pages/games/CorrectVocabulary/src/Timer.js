export default class Timer {
  constructor(scene, x, y) {
    ...
    this.timeText = this.scene.add.text(this.x, this.y, " ", {
  ...
  createTimer() {
    this.timeLabel = this.scene.add.text(
      ...
      this.timeText.y,
      "00:00",
      { font: "200px LuckiestGuy", fill: "#fff" }
    );
  }
  ...
  updateTimer(cb) {
    ...
    this.result = this.minutes < 10 ? this.minutes : this.minutes;
    this.result += this.seconds < 10 ? this.seconds : this.seconds;
